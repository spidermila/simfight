...
class Fighter(Unit):
        ...
        if isinstance(self.target, Unit):
            if self.accuracy >= randint(1, 100):
                ...
                if self.target.hp - dmg <= 0:
                    self.myworld.turn_log.append(
                        f'{self.name} attacking {self.target.name} ' +
                ...
                self.myworld.turn_log.append(
                    f'{self.name} attacking ' +
                    f'{self.target.name} ({self.target.hp} HP) - Missed',
                )
        ...
        if isinstance(self.target, Unit):
            ...
            while not can_move:
                ...
                for loc in self.surrounding_fields:
                    ...
                    if (
                        best_move_dst == dst and
                        self.myworld.square_is_valid(
                            self.x + loc[0],
                            self.y + loc[1],
                        )
                    ):
    ...
    def do_something(self) -> bool:
        ...
        if straight_dist > self.attack_max_range:
            ...
            # only move this turn, number of
            # fields moved depends on the unit's speed
